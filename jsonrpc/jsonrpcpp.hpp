...
public:
    ...
    Json InvokeMethod(jsonrpcpp::request_ptr request)
    {
        ...
        {
            std::cout << "not find method" << std::endl;
...
//0707 params is C#
        if (json.count("params1") != 0u)
            params_.parse_json(json["params1"]);
        else
            params_ = nullptr;
    }
...
inline entity_ptr Parser::do_parse(const std::string& json_str)
{
    ...
    {
        std::cout << "entity_ptr Parser::do_parse parse json failed !" << std::endl;
        throw ParseErrorException(e.what());
    }
